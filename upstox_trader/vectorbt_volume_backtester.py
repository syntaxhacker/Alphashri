#!/usr/bin/env python3
"""
VectorBT Volume Scanner Backtester
Advanced backtesting system for volume-based trading strategies
"""
...
class VolumeStrategyBacktester:
    """Advanced backtesting system using VectorBT"""
    ...
    def __init__(self):
        ...
        self.strategy_params = {
            ...
            'atr_threshold': 1.2,        # 1.2x normal ATR
            'rsi_entry_min': 50,         # RSI > 50 for momentum
            'rsi_entry_max': 75,         # RSI < 75 to avoid overbought
            'stop_loss_atr': 2.0,        # Stop loss = 2x ATR
            'take_profit_atr': 3.0,      # Take profit = 3x ATR
            'holding_period_max': 5      # Max 5 days holding
        }
        
        print("🚀 VectorBT Volume Strategy Backtester Initialized")
    ...
    def generate_volume_signals(self, indicators):
        """Generate trading signals based on volume strategy"""
        try:
            # Extract indicators
            rsi = indicators['rsi']
            atr = indicators['atr']
            ...
            # Entry conditions (Volume Momentum Strategy)
            volume_spike = volume_ratio > self.strategy_params['volume_threshold']
            rsi_momentum = (rsi > self.strategy_params['rsi_entry_min']) & (rsi < self.strategy_params['rsi_entry_max'])
            price_above_sma = close > sma_20
            macd_bullish = macd_hist > 0
            
            # Combine conditions for entry signals
            entry_signals = volume_spike & rsi_momentum & price_above_sma & macd_bullish
    ...
    def run_backtest_single_symbol(self, symbol, df):
        ...
        try:
            ...
            # Generate signals
            signals = self.generate_volume_signals(indicators)
            if signals is None:
                return None
    ...
    def run_comprehensive_backtest(self, symbols, period='1y', interval='1d'):
        ...
        print("🚀 Starting Comprehensive Volume Strategy Backtest")
        print("=" * 70)
        ...
        print(f"💰 Strategy: Volume Momentum + ATR Risk Management")
    ...
    def save_backtest_results(self, backtest_results, filename=None):
        ...
        try:
            if filename is None:
                ...
                filename = f"volume_backtest_results_{timestamp}"
            ...
            with open(f"{filename}_metrics.txt", 'w') as f:
                f.write("Volume Strategy Backtest Results\n")
                f.write("=" * 40 + "\n")
...
def main():
    ...
    print("🚀 VectorBT Volume Strategy Backtester")
    print("=" * 50)
    ...
    print(f"📅 Period: 1 year historical data")
    
    # Run comprehensive backtest
    results = backtester.run_comprehensive_backtest(test_symbols, period='6mo')
