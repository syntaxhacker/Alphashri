...
class TVScreenerUsage:
    def __init__(self, market='in', enable_paper_trading=False):
        ...
        self.trading_start_time = "10:00"  # Start trading at 10:00 AM (avoid early volatility)
        self.trading_end_time = "11:30"    # Stop trading at 11:30 AM (optimal window)
    ...
    def _check_historical_trend(self, symbol, timeframe='daily', lookback_days=20):
        ...
        try:
            ...
            # Calculate trend indicators - Fix: API returns datetime index, not timestamp column
            df = df.sort_index().reset_index()  # Convert datetime index to column
            ...
            try:
                ...
                if df is not None and not df.empty and len(df) >= 10:
                    # Simple trend analysis with available data - Fix datetime index
    ...
    def _process_paper_trading_alert(self, alert):
        ...
        try:
            ...
            # IMPROVED TREND CONFIRMATION: Block trades against trend (major improvement)
            if trend == 'neutral':
                console.print(f"[yellow]⚠️ TRADE BLOCKED: {symbol} - Neutral trend, no directional bias[/yellow]")
                return
            
            # Check volatility before entry to avoid high-risk stocks
            volatility_level = self._detect_volatility_level(symbol, price)
            if volatility_level == 'high':
                console.print(f"[yellow]⚠️ TRADE BLOCKED: {symbol} - High volatility stock (gaps through stop losses)[/yellow]")
                return
            
            # Determine trade direction with STRICT trend confirmation
            trade_side = None
            
            # Only allow BUY signals in bullish trends and SELL in bearish trends
            if trend in ['strong_bullish', 'bullish']:
                console.print(f"   [green]📈 {symbol} in {trend} trend - allowing BUY signals only[/green]")
                if alert['type'] == 'VOLUME_SPIKE':
                    # Volume spike with positive change = BUY (bullish trend only)
                    if alert.get('change', 0) > 0:
                        trade_side = 'BUY'
                elif alert['type'] == 'PRICE_MOVE':
                    # Strong positive price move = BUY (bullish trend only)
                    if alert.get('current_change', 0) > 2:
                        trade_side = 'BUY'
                        
            elif trend in ['strong_bearish', 'bearish']:
                console.print(f"   [red]📉 {symbol} in {trend} trend - allowing SELL signals only[/red]")
    ...
    def _execute_screener_trade(self, symbol, side, alert, price, quantity, confidence, trend='neutral'):
        ...
        try:
            ...
            # IMPROVED TIMING FILTER: Avoid high-volatility periods with poor entry success rates
            from datetime import datetime
            current_time = datetime.now().time()
            
            # Block entries during high-loss periods (based on analysis)
            early_cutoff = datetime.strptime("10:00", "%H:%M").time()  # Before 10:00 AM (77.8% loss rate)
            if current_time < early_cutoff:
                console.print(f"[yellow]⏰ TRADE BLOCKED: {symbol} - Early morning volatility period (before 10:00 AM)[/yellow]")
                return False
            
            # Block very late entries (after 11:30 AM - 100% loss rate)  
            late_cutoff = datetime.strptime("11:30", "%H:%M").time()
            if current_time > late_cutoff:
                console.print(f"[yellow]⏰ TRADE BLOCKED: {symbol} - Late entry period (after 11:30 AM)[/yellow]")
                return False
            ...
            # Volatility level already checked above - use normal volatility for filtered entries
            volatility_level = 'normal'  # Since we filtered out high volatility stocks
    ...
    def _fetch_price_from_exchange(self, symbol, exchange):
        ...
        try:
            ...
            current_price = self.upstox_api.get_current_price_with_streaming(
                symbol=symbol,
                instrument_type='EQ',
    ...
    def _monitor_position_risk(self, symbol, position):
        ...
        try:
            ...
            # Risk Management Rules - STRICT 0.5% stop loss for ALL trades
            stop_loss_pct = -0.5  # 0.5% strict stop loss for all trades
