...
describe("loadBot", () => {
  ...
  it("fetches single bot and sets selectedBot", async () => {
    const api = await import("../api/bots");
    const bot = createMockBot({ id: "bot-1", name: "Loaded Bot" });
    vi.mocked(api.getBot).mockResolvedValue(bot);

    await loadBot("bot-1");

    expect(getBotsState().selectedBot).toEqual(bot);
    expect(getBotsState().loading.load).toBe(false);
  });

  it("sets error on failure", async () => {
    const api = await import("../api/bots");
    vi.mocked(api.getBot).mockRejectedValue(new Error("Not found"));

    await loadBot("bot-1");

    expect(getBotsState().error).toBe("Not found");
    expect(getBotsState().loading.load).toBe(false);
  });
});

describe("loadBotStatus", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    stopAutoRefresh();
  });
  ...
  it("fetches status", async () => {
    const api = await import("../api/bots");
    vi.mocked(api.getBotStatus).mockResolvedValue({ running: true, pid: 123 } as any);

    await loadBotStatus("bot-1");

    expect(getBotsState().botStatus?.running).toBe(true);
    expect(getBotsState().loading.status).toBe(false);
  });

  it("handles error gracefully", async () => {
    const api = await import("../api/bots");
    vi.mocked(api.getBotStatus).mockRejectedValue(new Error("fail"));
...
describe("loadBotTrades", () => {
  ...
  it("fetches trades", async () => {
    const api = await import("../api/bots");
    vi.mocked(api.getBotTrades).mockResolvedValue({ trades: [{ id: 1, symbol: "TCS" }] } as any);

    await loadBotTrades("bot-1");

    expect(getBotsState().botTrades).toHaveLength(1);
    expect(getBotsState().loading.trades).toBe(false);
  });

  it("handles error gracefully", async () => {
    const api = await import("../api/bots");
    vi.mocked(api.getBotTrades).mockRejectedValue(new Error("fail"));

    await loadBotTrades("bot-1");

    expect(getBotsState().botTrades).toEqual([]);
    expect(getBotsState().loading.trades).toBe(false);
  });
});

describe("loadAvailableStrategies", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    stopAutoRefresh();
  });
  ...
  it("loads available strategies", async () => {
    const api = await import("../api/bots");
    const strategies = [{ id: "1", name: "ORB" }];
    vi.mocked(api.listAvailableStrategies).mockResolvedValue(strategies as any);

    await loadAvailableStrategies();

    expect(getBotsState().availableStrategies).toEqual(strategies);
    expect(getBotsState().loading.strategies).toBe(false);
  });
});
...
describe("updateBotAction", () => {
  ...
  it("updates bot and reloads list", async () => {
    const api = await import("../api/bots");
    const updatedBot = createMockBot({ name: "Updated" });
    vi.mocked(api.updateBot).mockResolvedValue(updatedBot);
    vi.mocked(api.listBots).mockResolvedValue([updatedBot]);

    const result = await updateBotAction("bot-1", { name: "Updated" });

    expect(result).toEqual(updatedBot);
    expect(getBotsState().showEditModal).toBe(false);
    expect(getBotsState().editingBot).toBeNull();
  });

  it("returns null on error", async () => {
    const api = await import("../api/bots");
    vi.mocked(api.updateBot).mockRejectedValue(new Error("update fail"));

    const result = await updateBotAction("bot-1", { name: "Updated" });

    expect(result).toBeNull();
    expect(getBotsState().error).toBe("update fail");
  });
});

describe("initBotsState", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    stopAutoRefresh();
  });
  ...
  it("calls loadBots only on first call", async () => {
    const api = await import("../api/bots");
    vi.mocked(api.listBots).mockResolvedValue([]);
    const callsBefore = vi.mocked(api.listBots).mock.calls.length;

    initBotsState();

    expect(vi.mocked(api.listBots).mock.calls.length).toBe(callsBefore + 1);
