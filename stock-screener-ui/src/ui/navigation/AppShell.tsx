...
import type { UIAppShellProps, UIAppShellHeaderProps, UIAppShellNavbarProps, UIAppShellMainProps } from "../types";

export function AppShell({ header, navbar, padding, layout, children, className, style, "data-testid": testId, ...rest }: UIAppShellProps) {
  return <MantineAppShell header={header as any} navbar={navbar as any} padding={padding} layout={layout} className={className} style={style} data-testid={testId} {...rest}>{children}</MantineAppShell>;
}
...
export function AppShellSection({ children, className, style, "data-testid": testId, ...rest }: UIAppShellMainProps) {
