...
export function Menu({ trigger, opened, onChange, position, offset, withArrow, shadow, closeOnItemClick, closeOnClickOutside, loop, children, renderTarget, className, style, "data-testid": testId, ...rest }: UIMenuProps) {
  return <MantineMenu trigger={trigger} opened={opened} onChange={onChange} position={position} offset={offset} withArrow={withArrow} shadow={shadow} closeOnItemClick={closeOnItemClick} closeOnClickOutside={closeOnClickOutside} loop={loop} className={className} style={style} data-testid={testId} {...rest}>{children}</MantineMenu>;
}

export function MenuTarget({ children, className, style, "data-testid": testId, ...rest }: UIMenuTargetProps) {
  return <MantineMenu.Target className={className} style={style} data-testid={testId} {...rest}>{children}</MantineMenu.Target>;
}
