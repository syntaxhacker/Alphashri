...
export function Tabs({ value, defaultValue, onChange, variant, color, orientation, activateOnFocus, loop, children, className, style, "data-testid": testId, ...rest }: UITabsProps) {
  return <MantineTabs value={value} defaultValue={defaultValue} onChange={onChange} variant={variant} color={color} orientation={orientation} activateOnFocus={activateOnFocus} loop={loop} className={className} style={style} data-testid={testId} {...rest}>{children}</MantineTabs>;
}

export function TabsList({ children, className, style, "data-testid": testId, ...rest }: UITabsProps) {
  return <MantineTabs.List className={className} style={style} data-testid={testId} {...rest}>{children}</MantineTabs.List>;
}

export function Tab({ value, icon, rightSection, disabled, children, className, style, "data-testid": testId, ...rest }: UITabProps) {
  return <MantineTabs.Tab value={value} icon={icon} rightSection={rightSection} disabled={disabled} className={className} style={style} data-testid={testId} {...rest}>{children}</MantineTabs.Tab>;
}
