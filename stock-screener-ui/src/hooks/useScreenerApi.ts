...
export function useScreenerPreview(activeScreener: string, columns?: string[], filters?: Record<string, any>): {
  stocks: Stock[];
  loading: boolean;
  error: Error | null;
  refresh: () => void;
  ...
  const paramsRef = useRef({ activeScreener, columns, filters });

  useEffect(() => {
    paramsRef.current = { activeScreener, columns, filters };
  ...
  const state = useApi<{ approaching: Stock[]; touched: Stock[] }>({
    url: buildPreviewUrl,
    immediate: true,
  });
  ...
  useEffect(() => {
    if (state.data) {
      console.log("[useScreenerPreview] got data:", state.data);
      setStocks(state.data.approaching || []);
    }
    if (state.error) {
      console.log("[useScreenerPreview] error:", state.error);
    }
