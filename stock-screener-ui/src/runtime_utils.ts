...
export function detectAddedSymbols(
  ...
  if (prev.provider !== next.provider || prev.mode !== next.mode || prev.screener !== next.screener) {
...
export function buildProfileFilterQueryParams(values: Record<string, string | number>): string {
...
export function getTradingList(stocks: Array<{ symbol: string }>): string {
