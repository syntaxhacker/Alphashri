...
export function PaperPortfolioCard({ portfolio, isMultiStrategy, strategySummaries }: PaperPortfolioCardProps) {
  if (!portfolio) {
    return (
      <Card shadow="sm" padding="md" radius="md" withBorder data-testid="portfolio-card">
        <Text c="dimmed" ta="center">Loading portfolio...</Text>
      </Card>
    );
  }
    ...
    <Card shadow="sm" padding="md" radius="md" withBorder data-testid="portfolio-card">
      <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md" data-testid="portfolio-row-1">
        <Group gap="xs">
          ...
          <Text size="md" fw={600}>₹{formatCurrency(portfolio.total_value)}</Text>
        </Group>
        <Group gap="xs">
          <Text size="xs" c="dimmed">Cash</Text>
          <Text size="md" fw={600}>₹{formatCurrency(portfolio.cash)}</Text>
        </Group>
        <Group gap="xs">
          <Text size="xs" c="dimmed">Margin Used</Text>
          <Text size="md" fw={600}>₹{formatCurrency(portfolio.margin_used)}</Text>
        </Group>
      </SimpleGrid>
      
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md" mt="md" data-testid="portfolio-row-2">
        <Group gap="xs">
          <Text size="xs" c="dimmed">Day P&L</Text>
          <Text size="md" fw={600} c={pnlColor}>
            {pnlSign}₹{formatCurrency(portfolio.day_pnl)}
          </Text>
        </Group>
        <Group gap="xs">
          <Text size="xs" c="dimmed">Positions</Text>
          <Badge variant="light" color="blue">{portfolio.positions_count}</Badge>
        </Group>
      </SimpleGrid>
