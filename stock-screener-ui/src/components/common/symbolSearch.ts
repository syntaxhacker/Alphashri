...
function selectSymbol(containerId: string, symbol: string): void {
  ...
  const input = document.querySelector(`[data-search-container="${containerId}"]`) as HTMLInputElement
  if (input) {
...
function renderDropdown(containerId: string): void {
  ...
  dropdown.innerHTML = state.results.map((result, index) => `
    <div
 ...
 */
export function registerSymbolSearchCallback(containerId: string, callback: (symbol: string) => void): void {
