    ...
    for i, b in enumerate(base):
        ...
        if in_pos:
            for tk in by_min.get(t, []):
                ...
                # V2: breakeven move at +0.5R (5/13 base losers reached +0.5R then reversed to SL)
                if not p.get("be") and p["mfe"] >= 0.5 * p["risk"]:
                    p["sl"] = p["entry"]
                    p["be"] = True
                if p["side"] == "SHORT":
                    if ask >= p["sl"]:
            ...
            if "exit" in in_pos:
                ...
                if p.get("be") and p["result"] == "SL" and abs(p["pnl"]) <= max(3.0, 0.15 * p["risk"]):
                    p["result"] = "BE"  # V2: stopped at breakeven, not a real loss
                trades.append(p)
                in_pos = None
                continue
        ...
        if window == "prime" and not prime:
            continue
        if (int(hm[:2]) * 60 + int(hm[3:])) - 19 * 60 <= 5:
            continue  # V2: skip first-5-min noise (5/13 base losers live here)
        ...
        if want == "SHORT":
            ...
            if risk < 5:
                continue
            if risk > 55:
                continue  # V2: wide-stop trades lose 7/7 in base sample
            tp = entry - risk  # 1R
        else:
...
def study(dates_csv, window):
    ...
    print(f"\n### LOSS STUDY n={len(tot)} W={len(winners)} L={len(losers)} net={sum(t['pnl'] for t in tot):+.1f}")
    print("\n-- losers: full detail (MFE/MAE in R) --")
    for t in sorted(losers, key=lambda x: x["pnl"]):
        print(f"  {t['date'][5:]} {t['hm']} {t['side']:5} {t['tf']:3} x{t['touches']} "
              f"risk {t['risk']:.0f}({t['risk_atr']:.1f}a) dist {t['dist_atr']:.1f}a "
              f"inv_age {t['inv_age']} min_open +{t['min_open']}m resid {t['resid']:.1f} "
              f"MFE {t['mfe_r']:+.2f}R MAE {t['mae_r']:+.2f}R -> {t['result']} {t['pnl']:+.1f}")
    # what-if: breakeven stop at +0.7R (SL->entry once MFE>=0.7)
    saved = sum(1 for t in losers if t["mfe_r"] >= 0.7)
    rec = sum(t["risk"] for t in losers if t["mfe_r"] >= 0.7)  # -1R -> ~0
    print(f"\n-- what-if breakeven@+0.7R: {saved}/{len(losers)} losers touched +0.7R first; "
          f"recovered ≈ +{rec:.0f} pts (losses -> scratch, winners untouched if MFE>=1 anyway) --")
    for cut, label in [(0.5, "BE@0.5R"), (0.7, "BE@0.7R"), (1.0, "TP-hit?")]:
        s = sum(1 for t in losers if t["mfe_r"] >= cut)
        print(f"   {label}: {s}/{len(losers)} losers reached it")
    print("\n-- losers by touches --")
    from collections import Counter
    print("   ", dict(Counter(t["touches"] for t in losers)), " vs winners ", dict(Counter(t["touches"] for t in winners)))
    print("-- losers by inv_age --")
    print("   ", dict(Counter(t["inv_age"] for t in losers)), " vs winners ", dict(Counter(t["inv_age"] for t in winners)))
    print("-- losers by min_open bucket --")
    def bkt(t):
        m = t["min_open"]
        return "0-5" if m <= 5 else ("6-15" if m <= 15 else "16-30")
    print("   ", dict(Counter(bkt(t) for t in losers)), " vs winners ", dict(Counter(bkt(t) for t in winners)))
    print("-- losers risk_atr>1.5? --")
    print("   losers:", sum(1 for t in losers if t["risk_atr"] > 1.5), "/", len(losers),
          " winners:", sum(1 for t in winners if t["risk_atr"] > 1.5), "/", len(winners))
    print("-- losers dist_atr>2? --")
    print("   losers:", sum(1 for t in losers if t["dist_atr"] > 2.0), "/", len(losers),
          " winners:", sum(1 for t in winners if t["dist_atr"] > 2.0), "/", len(winners))
    print("-- instant vs grind: losers MAE hit -1R while MFE<0.3R (never alive) --")
    dead = [t for t in losers if t["mfe_r"] < 0.3]
    print(f"   {len(dead)}/{len(losers)} never alive: " +
          ", ".join(f"{t['date'][5:]} {t['hm']} {t['side']}/{t['tf']}" for t in dead))
