...
# V3 context gates (from ctx study on IS-10: pd-outside 9/13 +340, val-inside 13/19 +331)
V3_PD_OUTSIDE = True   # entry must be outside prior-day range (initiative, not mid-range chop)
V3_VAL_INSIDE = True   # entry must be inside developing value (no outside-value chase)
...
def run_date(date, gap_min=2.0, sl_buf=6.0, window="prime", verbose=True):
    ...
    for i, b in enumerate(base):
        ...
        if not pick:
            continue
        # V3: context gate on close-proxy (final entry slips ~1-3pts; levels rarely flip)
        _px = b["close"]
        _prof = session_profile(ticks, t * 1000)
        if _prof is None or pdh is None:
            continue
        _val_loc = "above" if _px > _prof["vah"] else ("below" if _px < _prof["val"] else "inside")
        _pd_loc = "above" if _px > pdh else ("below" if _px < pdl else "between")
        if V3_PD_OUTSIDE and _pd_loc == "between":
            continue  # mid prior-range chop: IS 11W/9L net -267
        if V3_VAL_INSIDE and _val_loc != "inside":
            continue  # outside-value chase: IS outside net -258
        ...
        in_pos = {"side": want, "entry": entry, "sl": sl, "tp": tp, "risk": risk,
                  ...
                  "mfe": 0.0, "mae": 0.0}
        # CTX: reference location at signal time (reuse gate-time profile; blockers on final entry)
        prof = _prof
