...
def cmd_close(args):
    ...
    save_positions(positions)
    print(f"CLOSE #{args.id}: exit ₹{exit_p:.2f} → gross P&L ₹{gross:+,.2f}")
...
def cmd_pnl(args):
    ...
    total = 0.0
    print(f"{'id':>3} {'type':>4} {'strike':>9} {'qty':>4} {'entry':>9} {'ltp':>9} {'pnl':>11} {'status':>7}")
    for p in positions:
        if p["status"] == "OPEN":
            chain = fetch_chain(p["expiry"], token)
            ltp = get_contract_ltp(chain, p["strike"], p["type"], token)
        ...
        print(f"{p['id']:>3} {p['type']:>4} {p['strike']:>9,.0f} {p['qty']:>4} "
              f"{p['premium']:>9.2f} {ltp:>9.2f} {pnl:>+11,.2f} {p['status']:>7}")
