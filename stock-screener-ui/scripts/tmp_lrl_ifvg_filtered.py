...
def run_date(date, gap_min=2.0, sl_buf=6.0, window="prime", verbose=True):
    ...
    trades = []
    in_pos = None
    # TEMP FILTERS (experimental, not in base prototype):
    sl_count = 0           # day SL stop: no fresh entry after 2 SLs
    piv_high = None        # 1m BOS state (3-bar fractal, 2 each side)
    piv_low = None
    piv_high_broken = False
    piv_low_broken = False
    bos_dir = 0
    skipped = {"risk_cap": 0, "bos": 0, "daystop": 0}
    ...
    for i, b in enumerate(base):
        ...
        for tf in TF_ORDER:
            ...
            while ptr[tf] < len(bars) and bars[ptr[tf]]["time"] <= t:
                ...
                ptr[tf] += 1
        # TEMP: 1m BOS state (same fractal semantics as trading/smc_ifvg.py)
        if i >= 4:
            j = i - 2
            w = base[j - 2:j + 3]
            if len(w) == 5:
        ...
        if in_pos:
            ...
            if "exit" in in_pos:
                ...
                if p["result"] == "SL":
                    sl_count += 1  # TEMP day-stop counter
                trades.append(p)
                in_pos = None
                continue
        ...
        if sl_count >= 2:
            skipped["daystop"] += 1
            continue  # TEMP day-stop: done after 2 SLs
        ...
        if not pick:
            continue
        if pick == "3m":
            # TEMP: lone-3m must agree with 1m BOS (drop chop/counter-trend 3m)
            need = 1 if want == "LONG" else -1
            if bos_dir != need:
                skipped["bos"] += 1
                continue
        ...
        if want == "SHORT":
            ...
            if risk < 5:
                continue
            a_now = atr(base, i) or 5.0
            if risk > 60 or risk > 1.5 * a_now:
                skipped["risk_cap"] += 1
                continue  # TEMP risk cap
            tp = entry - risk  # 1R
        else:
    ...
    if verbose:
        print(f"\n=== {date} window={window} — {len(base)} 1m bars — {len(trades)} LRL-iFVG trades "
              f"(skip risk:{skipped['risk_cap']} bos:{skipped['bos']} daystop:{skipped['daystop']}) ===")
        for tr in trades:
